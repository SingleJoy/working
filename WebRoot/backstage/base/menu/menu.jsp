...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${pageContext.request.contextPath}/back_menu/treegrid.action',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
	...
	function edit() {
		...
		if (node) {
			var p = parent.sy.dialog({
				title : '修改菜单',
				href : '${pageContext.request.contextPath}/back_menu/to_menu_edit.action',
				width : 500,
				height : 280,
				buttons : [ {
					text : '修改',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/back_menu/edit_menu.action',
							success : function(d) {
				...
				onLoad : function() {
					...
					var ptree = pid.combotree({
						lines : true,
						url : '${pageContext.request.contextPath}/back_menu/doNotNeedSession_tree_recursive.action',
	...
	function append() {
		var p = parent.sy.dialog({
			title : '添加菜单',
			href : '${pageContext.request.contextPath}/back_menu/to_menu_add.action',
			width : 500,
			height : 280,
			buttons : [ {
				text : '添加',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/back_menu/save_menu.action',
						success : function(d) {
							var json = $.parseJSON(d);
							if (json.success) {
								treegrid.treegrid('reload');
	...
	function remove() {
		...
		if (node) {
			parent.sy.messagerConfirm('询问', '您确定要删除【' + node.name + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/back_menu/delete_menu.action',
						data : {
							id : node.id
						},
						cache : false,
