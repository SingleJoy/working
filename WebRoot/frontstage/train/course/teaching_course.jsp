        ...
        <div class="row row-3-9">
            ...
            <div class="col-md-9">
                <form id="courseForm" action="${pageContext.request.contextPath}/course/to_course.action" method="post">
                    ...
                    <input type="hidden" name="courseTitle" value="${courseView.courseTitle }"/>
                </form>
                ...
                <div class="panel panel-default panel-col lesson-manage-panel">
                    ...
                    <div class="panel-body">
                        <form class="form-horizontal form-inline well-sm pull-right" style="margin-bottom: 0;z-index: 999;">
                            <div class="form-group">


                                <input type="text" name="course_title_str" placeholder="搜索" class="form-control">

                                <input type="button" class="btn btn-primary btn-sm" value="关键字" onclick="searchCourse()">
                            </div>
                        </form>
                        <ul class="nav nav-tabs" style="margin-bottom: 0;max-width: 60%;">

                            <li class="<c:if test="${courseView.type==0}">active</c:if>"><a href="${pageContext.request.contextPath}/course/to_course.action?&stamp=1&state=12&sign=1&type=0&status=-1" >普通课程<c:if test="${!empty courseView.total&&courseView.type==0}">(${courseView.total})</c:if></a></li>
                            <li class="<c:if test="${courseView.type==2}">active</c:if>"><a href="${pageContext.request.contextPath}/course/to_course.action?&stamp=1&state=12&sign=1&type=2&status=-1" >直播课程<c:if test="${!empty courseView.total&&courseView.type==2}">(${courseView.total})</c:if></a></li>
                            ...
                            <c:forEach  items="${courseList}" var="course">
                                <tr >
                                    ...
                                    <td >
                                        ...
                                        <div class="mlm">
                                            <a href="javascript:void(0);" onclick="toCourseDetail(this,${course.id});" class="course_title_name" target="_blank" title="${course.title }">${fn:substring(course.title,0,16)}</a>
                                        </div>
                                    </td>
...
<script type="text/javascript">
    ...
    function searchCourse(){
    	var course_title_str = $("input[ name = 'course_title_str' ]").val();
    	if(course_title_str==null||course_title_str.trim()===""){
    		$("input[ name = 'courseTitle' ]").val("");
    	}else{
    		$("input[ name = 'courseTitle' ]").val(course_title_str.trim());
    	}
