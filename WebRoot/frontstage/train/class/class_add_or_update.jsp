...
<link rel="stylesheet" media="screen" href="${pageContext.request.contextPath}/frontstage/css/bootstrap/common.css" />
			...
			<div class="col-md-9">
				<div class="panel panel-default panel-col test-creator">
					<div class="panel-heading clearfix" >
						<c:choose>
							<c:when test="${empty classView.id }">创建课程班级</c:when>
							<c:otherwise>编辑课程班级</c:otherwise>
						</c:choose>
					</div>
					<div class="panel-body">
						...
						<form id="sub" class="form-horizontal" method="post" action="${pageContext.request.contextPath}/class/save_or_update.action">
							...
							<input name="sign" type="hidden" <c:if test="${classView.source_type==0}">value=14</c:if><c:if test="${classView.source_type==1}">value=10</c:if>>
							<input name="source_id" value="${classView.source_id}" type="hidden">
							<input name="source_type" value="${classView.source_type}" type="hidden">
							<c:if test="${!empty classView.id }">
								<input name="id" value="${classView.id}" type="hidden" >
							</c:if>

							<input name="classes" type="hidden" class="form-control"
							<c:choose>
							<c:when test="${!empty classView.id }">value="${classView.classes }"</c:when>
							<c:otherwise>value="${classes }"</c:otherwise>
							</c:choose> onfocus="classes.blur()">
							<%--</div>--%>
							<%--</div>--%>
							...
							<div class="form-group">
								...
								<div class="col-md-6 controls">
									<input class="form-control" name="name" id="name" value="${classView.name}">
							...
							<div class="form-group">
								...
								<div class="col-md-8 controls" id="tutor_list">
                                     <ul id="tutors" class="nav nav-pills clearfix tutor_list"></ul>
								</div>

								<span>
									<a href="javascript:void(0);" onclick="showTeacher(0)" class="tutor_add btn btn-info btn-sm" data-toggle="model">
							...
							<div class="form-group">
								...
								<div class="col-md-6 controls">
									<input class="form-control width-input width-input-large" id="inpt_credit" name="credit" value="0" type="number" min="0" step="1"> 分
							...
							<div id="no_edit" class="form-group" >
								...
								<div class="col-md-3 controls radios">
								...
								<div id="up" style="margin-left: 8px;margin-top: 20px;">
									...
									<div class="form-group" >
										...
										<div class="col-md-8 controls" >
											<input class="form-control width-input" id="inpt_maximum" name="maximum" value="0" type="number" min="0" step="1">&nbsp;&nbsp;人&nbsp;&nbsp;
											<div class="help-block" style="display: inline-block;" > 不设置学员上限则默认为无人数限制</div>
											<div class="maximum_val text-warning" style=" display: none;">请输入大于等于0的数字</div>
										</div>
									</div>
									...
									<div class="form-group" >
										...
										<div class="col-md-8 controls">
											<input class="form-control width-input " readonly="readonly" name="end_time" type="text"
												   <c:if test="${classView.sign_up==0&&!empty classView.end_time }">value="<fmt:formatDate value="${classView.end_time}" type="date"/>"</c:if> onClick="WdatePicker()" onfocus="WdatePicker()">
											<div style="display: inline-block;" class="help-block">不设置报名截止日期则默认为开班结束时间内均可报名</div>
											<div id="end_time0_msg" class="help-block"></div>
										</div>
									</div>
									...
									<%-- <div class="form-group">
										...
										<div id="is_refund" class="col-md-10 controls radios">
								...
								<div id="down" style="display: none;margin-left: 8px;">
									...
									<div class="form-group">
										...
										<div class="col-md-8 controls">
											...
											<div id="end_time1_msg"  class="help-block"></div>
										</div>
									</div>
							...
							<div class="form-group" style="margin-top: -40px;">
								...
								<div class="col-md-10 controls radios" >
									<label><input type="radio"  name="communityType" checked="checked" value="-1" onchange="queryCommunitys()">公开 </label>
									<label><input type="radio"  name="communityType" class="community" value="0" onchange="queryCommunitys()">社区 </label>
									<label><input type="radio"  name="communityType" class="workshop" value="1" onchange="queryCommunitys()">工作坊 </label>
									<div class="community_type_val_msg text-warning" style="display:none">请选择发布范围</div>
								</div>

							</div>

							<div class="community_content" style="display: none;">

								<div class="form-group">
									<div class="col-md-2 control-label"> <label>社区</label></div>
									<div class="col-md-10 controls">
										<select  class="form-control width-input width-input-large " name="communityId" onchange="queryClassifys()">
								...
								<div class="form-group">
									...
									<div class="col-md-10 controls">
										<select  class="form-control width-input width-input-large " name="classifyId">
							...
							<div class="workshop_content" style="display: none;">
							...
							<div class="form-group">
								...
								<div class="col-md-8 controls">
									<input class="form-control width-input " id="discount" name="discount" value="${classView.discount}" type="text" min="0" max="100" step="1">%
								...
								<div class="form-group">
									<div class="col-md-offset-2 controls" >
										<button type="button" class="btn btn-primary btn-fat" onclick="check();">
											<c:choose>
												<c:when test="${!empty classView.id }">编辑</c:when>
...
<script type="text/javascript">
	...
	$(function(){
		...
		$('input[name="end_time"]').blur(function(){
			...
			var checkTime3 = getTime(end_time);
			if(checkTime3!=""){
				if(checkTime3 - nowTime<=0){
					if(sign==0){
						$("#end_time0_msg").html("报名截止时间要晚于当前时间");
					}else if(sign==1){
						$("#end_time1_msg").html("密码有效期要晚于当前时间");
					}
					$(this).val("");
					return;
				}else{
				...
				if(checkTime2!=""){
					if(checkTime2 - checkTime3<0){
						if(sign==0){
							$("#end_time0_msg").html("报名截止时间要早于结束时间");
						}else if(sign==1){
							$("#end_time1_msg").html("密码有效期要早于结束时间");
						}
						$(this).val("");
						return;
					}else{
	...
	function check(){
		...
		if(community_type==="0"||community_type==="1"){
			...
			if(community_id==null||community_id===""){
				$(".community_val_msg").show()	;
				return;
			}else{
				$(".community_val_msg").hide();
			}
			if(classify_id==null){
				$(".classify_val_msg").show();
				return;
			}else{
				$(".classify_val_msg").hide();
			}
		...
		var maximumx = parseInt($("#inpt_maximum").val());
		if(isNaN(maximumx)||maximumx<0){
			$(".maximum_val").show();
			return false;
		}
		var sign = $('input[name="sign"]:checked').val();
