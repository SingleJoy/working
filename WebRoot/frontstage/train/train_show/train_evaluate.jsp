	...
	<c:if test="${empty user_appraise}">
		...
		<div class="evaluation_again_content">
			...
			<div class="form-group clearfix pull-right">
				...
				<button class="btn btn-primary pull-right" id="appraise_button" type="button" <c:if test="${isTeacher&&(trainDetailView.viewType==1||trainDetailView.viewType==2) }">disabled="disabled"</c:if>>发表</button>
			</div>
		</div>
	</c:if>
	...
	<c:if test="${!empty user_appraise }">
		...
		<div class="evaluation_again_content" style="display: none;">
			...
			<div class="form-group clearfix pull-right">
				...
				<button class="btn btn-primary" value="${user_appraise.id}" type="button" id="appraise_button" <c:if test="${isTeacher&&(trainDetailView.viewType==1||trainDetailView.viewType==2) }">disabled="disabled"</c:if>>发表</button>
			</div>
		</div>
	</c:if>
			...
			<div class="media media-evaluate">
				...
				<div class="media-body thread-post">
					<div class="title">
						...
						<span>
								<fmt:formatDate value="${app.appraise_time }" type="date" /> 
							</span>
					</div>
					...
					<div class="content">
						<div class="short-content">${app.appraise_content }
						</div>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        $("#pointer_more1").click(function(){
            ...
            $.ajax({
                type:'get',
                url : "${pageContext.request.contextPath}/user_study/ajax_appraise.action?id=${trainView.id}&type=1&page="+number,
                dataType:'json',
                success : function(response) {
                    if(!response.flag){
                        _this.remove();
