...
<div class="cn-wrap">
	...
	<div id="content-container" class="container" style="margin-top: 20px;">
		...
		<div class="tabs-wrapper">
			...
			<div class="tabs-group" id="sonClassify" style="<c:if test="${trainView.classify_id==-1}">display:none;</c:if>">
				<ul class="content clearfix" >
					<li <c:if test="${trainView.classify_id==-1}"> class="active" </c:if>>
						<%--<a class="nav-link" href="javascript:void(0)" onclick="doSonClassify(-1,'');" >全部</a></li>--%>
						<c:forEach items="${sonClassifyList}" var="sonClassify">
							<li <c:if test="${sonClassify.id==trainView.sonClassifyId}"> class="active" </c:if>>
		...
		<div class="panel panel-default"  style="margin-top: 20px;">
			<div class="panel-body">
				<form id="trainForm" action="${pageContext.request.contextPath}/train/to_train_center.action" method="post">
					...
					<input type="hidden" name="sonClassifyId" id="sonClassifyId" value="${trainView.sonClassifyId}"/>
			<input type="hidden" name="sonClassifyName" id="sonClassifyName" value="${trainView.sonClassifyName}"/>
