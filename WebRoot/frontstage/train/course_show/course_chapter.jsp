			...
			<c:forEach items="${hourList}" var="hour" varStatus="status">
				...
				<c:if test="${chapter.id==hour.parent_id}">
					<li class="period lesson-item " data-num="2" id="hour${status.index+1}list${chapter.id} ">
						<c:choose>
							<c:when test="${is_teacher&&(userStudyView.viewType==1||userStudyView.viewType==2) }">
						...
						<a onclick="doStudyHour(${hour.id});" href="javascript:void(0);" title="">${hour.hour_seq}
							<c:if test="${hour.attribute_type==2}">
								<span class="title" title="${hour.hour_title}">课时：${hour.hour_title}</span>
...
<script type="text/javascript">
	...
	function doStudyHour(hourId){
		if("${userStudyStatus==-1}"=="true"){
			...
			var tag_href;
			if(("${userStudyView.viewType }"==='1')&&("${is_teacher }"==='true')){
				tag_href="${pageContext.request.contextPath}/course_study/to_study_preview.action?id="+hourId+"&source_type=${userStudyView.sourceType}&learn_id=${userStudyView.learnId}";		
			}else{
				tag_href="${pageContext.request.contextPath}/course_study/to_study.action?id="+hourId+"&source_id=${userStudyView.sourceId}&source_type=${userStudyView.sourceType}&learn_id=${userStudyView.learnId}&user_study_id=${userStudyView.id}";
			}
			if("${userStudyView.is_direct}"==="1"){
				tag_href+="&is_direct=1&trainType=${userStudyView.trainType}";	
			}
