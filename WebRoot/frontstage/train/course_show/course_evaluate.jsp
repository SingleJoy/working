...
<div class="clearfix">
	...
	<div class="form-group controls" style="margin-top: 20px;">
			...
			<div class="form-group clearfix pull-right">
				...
				<button class="btn btn-primary pull-right" id="appraise_button" type="button" <c:if test="${is_teacher&&userStudyView.viewType==1 }">disabled="disabled"</c:if>>发表</button>
			</div>
		</div>
	</c:if>
		...
		<div class="evaluation_again_content" >
			...
			<c:forEach end="5" begin="1" varStatus="varStatus">
				<c:choose>
					...
					<c:otherwise>
						<img class="score_star" src="${pageContext.request.contextPath}/frontstage/images/score_star_icon.png"
							 name="${pageContext.request.contextPath}/frontstage/images/">
					</c:otherwise>
				</c:choose>
			...
			<div class="form-group controls" style="margin-top: 30px;">
				<textarea class="form-control" rows="8" name="appraise_content" id="appraise_content">${user_appraise.appraise_content }</textarea>
			</div>

			<div class="form-group pull-right">
				<a class="btn btn-link ">取消</a>
				<button class="btn btn-primary" value="${user_appraise.id}" type="button" id="appraise_button">发表</button>
			</div>
		</div>
		</div>
	</c:if>
	...
	<c:if test="${!empty appraise }">
				...
				<div class="media media-evaluate">
					...
					<div class="media-body thread-post clearfix">
						...
						<div class="content">
							...
							<div class="full-content" style="display:none;">${app.appraise_content }</div>
						</div>
					</div>
		...
		<c:if test="${totalAppraise>10}">
			...
			<div class="empty"  id="pointer_more1" >加载更多</div>
			<input type="hidden" id="pointer_number1" value="2">
		</c:if>

	</c:if>
	<input type="hidden" name="score" value="${user_appraise.score}" id="score_value">
	<input type="hidden" name="score" value="${courseView.subtitle}" id="course_subtitle">
...
//	评价标签点击样式
	...
	$(function() {
		...
		$("#pingjia").click(function() {
			$(".evaluation_again_content").toggle();
			$(this).toggle();
		});
		//点击取消
		...
		$("#pointer_more1").click(function(){
			...
			$.ajax({
				type:'get',
				url : "${pageContext.request.contextPath}/user_study/ajax_appraise.action?id=${courseView.id}&type=0&page="+number,
				dataType:'json',
				success : function(response) {
					if(!response.flag){
						_this.remove();
