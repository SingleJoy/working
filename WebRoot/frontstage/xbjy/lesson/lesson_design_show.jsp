                ...
                <c:if test="${(tab==0&&lessonLeft.plan101==1)||(tab==1&&lessonLeft.plan102==1)||(tab==3&&lessonLeft.prepare201==1)||(tab==4&&lessonLeft.assessment401==1)||(tab==2&&lessonLeft.plan102==1) }">
                    <div class="panel panel-default panel-col course-data">
                        <div class="panel-heading">教学设计与参考
                            <c:if test="${flag }">
                                <c:if test="${update==1 }">
                                    <a  class="btn btn-warning btn-fat pull-right" href="${pageContext.request.contextPath}/lesson_design/update_design.action?id=${lessonDesign.id}&lesson_id=${lessonView.id}&stamp=1&state=${state}&tab=${tab}">
                                        <c:if test="${tab==1 }">
                                            协同修改</c:if>
                                        <c:if test="${tab==0 }">
                                            修改教学设计及参考
                        ...
                        <div class="panel-body">
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped all-statistics" >
                                    ...
                                    <c:if test="${tab==1 }">
                                        <table class="table table-striped member-statistics" style="margin-top: 50px;">
                                            <tbody>
                                            <tr>
                                            ...
                                            <c:forEach items="${lessonDesignViews }" var="view" varStatus="status">
                                                <tr>
                                                    ...
                                                    <td >
                                                        ...
                                                        <a href="${pageContext.request.contextPath}/lesson_design/to_lesson_design.action?id=${view.id }&lesson_id=${lesson.id}&tab=2" target="_blank">查看内容</a><span class="mlm mrm">｜</span>
                                                        <c:if test="${status.last }">
                                                            <a href="javascript:void (0);" style="color: black">对比上一版本</a><span class="mlm mrm">｜</span>
                                                        </c:if>
                                                        <c:if test="${!status.last }">
                                                            <a href="${pageContext.request.contextPath}/lesson_design/compare.action?id=${view.id }&tab=${tab }&compare=before" target="_blank">对比上一版本</a><span class="mlm mrm">｜</span>
                                                        </c:if>
                                                        <c:if test="${status.last }">
                                                            <a href="javascript:void (0);" style="color: black">对比原始版本</a><span class="mlm mrm">｜</span>
                                                        </c:if>
                                                        <c:if test="${!status.last }">
                                                            <a href="${pageContext.request.contextPath}/lesson_design/compare.action?id=${view.id }&tab=${tab }&compare=first" target="_blank">对比原始版本</a><span class="mlm mrm">｜</span>
                                                        </c:if>
                                                        <c:if test="${status.first }">
                                                            <a href="javascript:void (0);"  style="color: black">对比最终版本</a>
                                                        </c:if>
                                                        <c:if test="${!status.first }">
                                                            <a href="${pageContext.request.contextPath}/lesson_design/compare.action?id=${view.id }&tab=${tab }&compare=last" target="_blank">对比最终版本</a>
                                                        </c:if>
                                                    </td>
                                                </tr>
                                            </c:forEach>
