...
//		if(classify.getParent_id().equals("")){//视为大类
//			classify.setParent_id("0");
//			String currType = classifyMapper.getLastType();
//			if(currType!=null&&currType.length()>0){
//				classify.setType(Integer.parseInt(currType)+1+"");
//			}else{
//				classify.setType("0");
//			}
//		}
	...
	public List<ClassifyView> treegrid(Classify classify) {
		...
		if (treeList != null && treeList.size() > 0) {
			if(classify.getId()==null){
				classify.setId(treeList.get(0).getId());
				treeList = classifyMapper.queryTreeGrid(classify);
			}
...
//			}
	...
	@Override
	public List<cn.bnsr.edu_yun.frontstage.base.view.ClassifyView> queryViewByParentId(Long classifyId) {
		
		return classifyMapper.queryViewByParentId(classifyId);
	}

	@Override
	public List<ClassifyView> queryparam() {
		Classify classify = new Classify();
		return classifyMapper.queryTreeGrid(classify);
	}
