...
public class UserStudyServiceImpl implements UserStudyService {
			...
			if(studyView.getSource_type()==0){
				for(StudyView st:studyList){
					...
					doing = userClassHourService.countIsNotStudy(uchv);
					 st.setAllNum(isNum+notNum+doing);
	...
	public boolean joinStudy(ClassView classView, User user) throws Exception {
		...
		if(userStudyIsPay == null){//未报名-->报名
			...
			//userStudy.setStatus(3);//报名---改:跳过报名状态直接进入开始学习状态
			userStudy.setStatus(0);//开始学习
	...
	@Override
	public List<UserStatsView> queryStuArea(UserStatsView condition) {
		return userStudyMapper.queryStuArea(condition);
	}

	@Override
	public List<UserStatsView> queryAgeByStu(UserStatsView condition) {
		List<UserStatsView> list= userInfoMapper.queryAgeByStu(condition);
		for(UserStatsView u :list){
			if("55".equals(u.getAgeInterval())){
				u.setAgeInterval("55以上");
				break;
			}
		}
		return list;
