...
<script type="text/javascript" charset="utf-8">
	...
	function edit() {
		...
		if (node) {
			var p = parent.sy.dialog({
				title : '修改菜单',
				href : '${pageContext.request.contextPath}/back_area/to_area_edit.action',
				width : 500,
				height : 280,
				buttons : [ {
					text : '修改',
					handler : function() {
						var f = p.find('form');
						f.form('submit', {
							url : '${pageContext.request.contextPath}/back_area/edit_subject.action',
							success : function(d) {
	...
	function append() {
		var p = parent.sy.dialog({
			title : '添加菜单',
			href : '${pageContext.request.contextPath}/back_area/to_area_add.action',
			width : 500,
			height : 280,
			buttons : [ {
				text : '添加',
				handler : function() {
					var f = p.find('form');
					f.form('submit', {
						url : '${pageContext.request.contextPath}/back_area/save_area.action',
						success : function(d) {
							var json = $.parseJSON(d);
							if (json.success) {
								treegrid.treegrid('reload');
	...
	function remove() {
		...
		parent.sy.messagerAlert('提示', '只能删除字节点', 'error');
			...
			parent.sy.messagerConfirm('询问', '您确定要删除【' + node.name + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/back_area/delete_area.action',
						data : {
							id : node.id
