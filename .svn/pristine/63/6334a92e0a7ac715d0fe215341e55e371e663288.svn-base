...
<div class="cn-section course-manage-header clearfix" style="border: none;margin-top: 20px;">
    ...
    <h1 class="title" >
        <a class="link-dark" href="javascript:void(0);" onclick="toCourseDetail(${trainView.id});" title="${trainView.name}">${trainView.name}</a>
    ...
    <div class="toolbar hidden-xs">
        ...
        <div class="btn-group">
            ...
            <ul class="dropdown-menu pull-right">
            	...
            	--%>
            	<li><a href="${pageContext.request.contextPath}/train/to_train_detail.action?id=${trainView.id}&sign=0&viewType=1">作为 已购买用户</a></li>
                <li><a href="${pageContext.request.contextPath}/train/to_train_detail.action?id=${trainView.id}&sign=0&viewType=2">作为 未购买用户</a></li>
            </ul>
        </div>
                ...
                success:function(data){
                	if(data.set_check==0){
                		if(data.train_of_num>0){
                			window.location.href="${pageContext.request.contextPath}/train/save_train.action?id="+${trainView.id}+"&sign="+${trainView.sign }+"&flag=2&is_check=1&status=1&type="+${trainView.type};
                		}else{
                			 if(data.trainType==0){
 	                        	if(data.train_of_num<=0){
                    	...
                    	if(data.train_of_num>0&&data.ck_total_required>0){
                        	window.location.href="${pageContext.request.contextPath}/train/save_train.action?id="+${trainView.id}+"&sign="+${trainView.sign }+"&flag=2&is_check=0&status=1&type="+${trainView.type};
                   	 	}else{
	                        if(data.trainType==0){
	                        	if(data.train_of_num<=0){
	                            	alert("该培训一个课程都没有，请先创建课程！");
