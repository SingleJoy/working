				...
				if (!(methodName.contains("to_") || methodName.equals("doNotNeedSession_tree"))) {
					...
					for (Method method : methods) {
						...
						if (methodName.equals(method.getName()) && method.isAnnotationPresent(UserLog.class)) {
							...
							if (!methodName.equals("loginByPhone")){
						...
						//	if (HttpRequest.checkAgentIsMobile(request)) {// 是手机端
								if (os.toLowerCase().contains("android")) {
									...
									os = "Android操作系统";
								} else if (os.toLowerCase().contains("os")) {
							...
							if (logger.isDebugEnabled()) {//
								...
								User user =null;
								if(session!=null)
								 user = (User) session.getAttribute("currentUser");// 此session对象只在前台存在
								if (methodName.equals("loginByPhone")) {
									String username = request.getParameter("username");
									String password = MD5.MD5c(request.getParameter("password"));
									user = new User();
								...
								} else if (methodName.equals("loginByPhoneYzm")) {
									session = request.getSession();
								...
								if (user != null) {// 前台用户已经登录！
									...
									if ("webService_login".equals(methodName)) {// 手机端登录
										/*
										 * //解析json参数包 JSONObject resJson =
										 * AppJson
										 * .toJson(request.getParameter("jsondata"
										 * )); userName =
										 * resJson.getString("loginName");
										 * userPassword =
										 * resJson.getString("loginPwd");
										 */
									} else {// WEB端登录
											// 获取登录用户名
											// userName =
								 ...
								 * else{//获取后台登录信息 //获取登录用户名 userName =
								 * request.getParameter("username"); //用户密码
								 * userPassword =
								 * request.getParameter("password"); }
								 */
								...
								if (methodName.equals("loginByPhone") || methodName.equals("loginByPhoneYzm")) {
									syslog.setLog_type(1);
									if (user == null) {
										syslog.setMsg(os + "登录失败");
									} else {
										syslog.setMsg(os + methodDescp);
									}
								} else if (methodName.equals("registerM")) {
									if (user == null) {
										syslog.setMsg(os + "注册失败");
									}else{
										syslog.setMsg(os + "注册成功");
									}
								}
								// 添加日志
								sysLogService.saveSysLog(syslog);
