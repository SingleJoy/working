				...
				<div class="panel panel-default panel-col">
					...
					<div class="panel-body">
						<form  class="form-horizontal" id="courseExamStandardAdd" action="${pageContext.request.contextPath}/course_exam_standard/update_course_exam_standard.action" method="post"  >
							<input type="hidden" id="courseId" name="courseId" value="${courseView.id }"/>
							<input type="hidden" id="sourceType" value="1"/>
							<input type="hidden" name="sign" value="${courseView.sign }"/>
							<input id="is_every" name="is_every" value="0" type="hidden"/>
							<p>各项考核占比及合格分数</p>
							<table class="table table-striped" style="text-align: center;">
								...
								<c:forEach items="${jobStandard}" var="job">
									...
									<tr>
										...
										<td>
											<input type="text" class="form-control width-input" name="ratio" value="${job.ratio }" style="width: 60px;">
										</td>
									</tr>
								</c:forEach>
								<c:forEach items="${discussionStandard}" var="discussion">
									<tr>
										<td><input name="id" value="${discussion.id }" hidden="hidden">主题讨论</td>
										<td >100</td>
										<td>${discussion.source_name }</td>
										<td>
											<input type="text" class="form-control width-input" name="ratio" value="${discussion.ratio }" style="width: 60px;">
										</td>
									</tr>
								</c:forEach>
								<c:forEach items="${examStandard}" var="exam">
									<tr>
										...
										<td>
											<input type="text" class="form-control width-input " name="ratio" value="${exam.ratio }" style="width: 60px;">
										</td>
									</tr>
								</c:forEach>
								</tbody>
							...
							<table class="table table-striped" style="margin-top: 30px;">
								<thead>
								<tr>
									<th width="4%" style="text-align: center;"><input class="checkAll subCheckbox" type="checkbox" autocomplete="off"></th>
								...
								<c:forEach items="${certificatesViews}" var="certificate" varStatus="i" >
									<tr id="row_${i.index}">
										<td><input class="subCheckbox" onclick="choosedel(this,${i.index},${certificate.certificate_id });" type="checkbox" autocomplete="off"/></td>
										<td>
											<input type="hidden"  name="certificate_id" value="${certificate.certificate_id }"/>
											<input type="hidden"  name="source_id" value="${courseView.id }"/>
											<input type="hidden"  name="source_type" value="${certificate.source_type }"/>
											<input name="down_score" id="down_score_${i.index}" value="${certificate.down_score }" type="text" class="form-control width-input" onblur="check_down_score(${i.index})" style="width: 100px;<c:if test="${courseView.status ==1 }">border:0px;</c:if>">
											——
											<input name="up_score" id="up_score_${i.index}" value="${certificate.up_score }" type="text" class="form-control width-input" onblur="check_up_score(${i.index})" style="width: 100px;<c:if test="${courseView.status ==1 }">border:0px;</c:if>">
											<input name="cert_model_id" id="cert_model_id_${i.index}" value="${certificate.cert_model_id }" type="hidden">
...
<script type="text/javascript">
	...
	var certids = [];//仅用于删除 证书id
	var certindexs = [];//仅用于删除 证书下标
	...
	function delRows(){//也要删除证书模板id
		...
		for(var i=0;i<indexlength;i++){
			var value = $("#cert_model_id_"+certindexs[i]).val();
			certIds.splice($.inArray(value,certIds),1);//删除证书模板id
			rangMapp.remove("rang"+certindexs[i]);
		...
		if(idlength>0){
			$.ajax({
				...
				dataType:'json',
				success:function(data){
					certids.splice(0,certids.length);
					certindexs.splice(0,certindexs.length);
				}
			});
		}
