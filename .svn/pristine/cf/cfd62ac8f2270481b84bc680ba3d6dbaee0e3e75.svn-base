		...
		<div class="row">
			...
			<div class="col-md-9">
				<div class="panel panel-default panel-col">
					...
					<c:if test="${examStandardMessage!=null }">
						...
						<form id="trainExamStandard"  class="form-horizontal" action="${pageContext.request.contextPath}/train_exam_standard/transaction_train_exam_standard.action" method="post"  >
							...
							<div class="form-group" style="clear: both;">
								...
								<div class="col-md-5 controls">
									...
									<label><input type="text" <c:if test="${trainView.type==1}">readonly="readonly"</c:if> id="total_required" class="form-control width-input"  name="total_required" value="${trainExamStandardView.total_required }" <c:if test="${trainView.status ==1 }">disabled="disabled"</c:if>></label>学时

								</div>
								<div id="total_required_msg" class="help-block" style="display:none;">
									请输入大于等于必修课时小于等于总学时的数
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-md-4 controls" >
									...
									<input id="cert_model_id" name="cert_model_id" value="${certificate.cert_model_id}" type="hidden">
...
<script type="text/javascript">
	...
	function checkform(){
		...
		var total_required = parseInt($("#total_required").val());
		var total = '${trainExamStandardView.totalHours}';
		var required = '${trainExamStandardView.requiredHours}';
		if(isNaN(total_required)||total_required<=0){
			//alert("总学时为非法字符或小于或等于0的值");
			$('#total_required_msg').text("总学时为非法字符或小于或等于0的值");
			$('#total_required_msg').show();
			return false;
		}else if(parseInt(required)>total_required || total_required>parseInt(total)){
			//alert("总学时必须大于等于必修课时小于等于必修与选修课的总学时");
			$('#total_required_msg').text("总学时必须大于等于必修课时小于等于必修与选修课的总学时");
			$('#total_required_msg').show();
			return false;
		}
		$('#total_required_msg').hide();
