                                ...
                                <c:if test="${!empty currentUser }">
                                    <c:if test="${userStudyStatus == 3}">
                                        <div style="margin-left: 20px;font-size: 16px;">
                                            已选<span class="text-danger">&nbsp;
                                         	<c:choose>
                                         		<c:when test="${empty userTrainClassView.course_total }">
                                         			0
                                         		</c:when>
                                         		<c:otherwise>
                                         			${userTrainClassView.course_total }
                                         		</c:otherwise>
                                         	</c:choose>
                                         	&nbsp;</span>门课
                                            <span class="mlm mrm">｜</span>
                                            已学<span class="text-danger">&nbsp;
                                            <c:choose>
                                         		<c:when test="${empty userTrainClassView.finish_hours_total }">
                                         			0
                                         		</c:when>
                                         		<c:otherwise>
                                         			${userTrainClassView.finish_hours_total }
                                         		</c:otherwise>
                                         	</c:choose>
                                            &nbsp;</span>学时,
                                            还剩<span class="text-danger">&nbsp;
                                            <c:choose>
                                         		<c:when test="${empty userTrainClassView.unfinish_hours_total }">
                                         			0
                                         		</c:when>
                                         		<c:otherwise>
                                         			${userTrainClassView.unfinish_hours_total }
                                         		</c:otherwise>
                                         	</c:choose>
                                            &nbsp;</span>学时
                                        </div>
                                    ...
                                    <c:forEach items="${trainCourseList}" var="tcv">
                                        <div class="course-item">
                                            <div class="media">
                                                ...
                                                <a class="media-left" id="train_course_0_${tcv.course_id}" onclick="doSearchCourse(${tcv.course_id},0);" href="javascript:void(0);">
                                                    <span class="tags">
                                                ...
                                                <div class="media-body">
                                                    <div class="title">
                                                        <a id="train_course_0_${tcv.course_id}" onclick="doSearchCourse(${tcv.course_id},0);" href="javascript:void(0);" >${tcv.title }</a>
                                                    </div>
                                                    ...
                                                    <div class="price hidden-xs">
                                                        <c:if test="${tcv.is_join==1 && tcv.is_required==1 && userStudyStatus == 3}">
                                                            <a href="javascript:void(0);" <c:if test="${!isTeacher||(trainDetailView.viewType!=1&&trainDetailView.viewType!=2) }">onclick="joinOptionCourse(${tcv.course_id});"</c:if> class="btn btn-info btn-sm">加入课程</a>
                                                        </c:if>
                                                        ...
                                                        <c:forEach items="${hourList}" var="hour" varStatus="status">
                                                            ...
                                                            <c:if test="${chapter.id==hour.parent_id}">
                                                                <li class="period lesson-item " style="display: list-item;">
                                                                    <a id="train_course_0_${tcv.course_id}" onclick="doSearchCourse(${tcv.course_id},0);" href="javascript:void(0);">                                                                    <!-- <i class="es-icon es-icon-doing color-primary status-icon"></i> -->
                                    ...
                                    <div class="col-lg-4 col-md-6 col-sm-6">
                                        <div class="teacher-item" >
                                            <div class="teacher-top">
                                                ...
                                                <div class="title">
                                                    <a class="link-dark" href="#">${teacher.teacherName}</a>
                                            ...
                                            <div class="teacher-bottom">
                                                ...
                                                <div class="metas">
                                                    ...
                                                    <a class="btn btn-default btn-sm unfollow-btn" href="javascript:void(0);"  style="display:none;">已关注</a>
                                                    <a class="btn btn-default btn-sm text-center" data-toggle="modal" data-target="#modal" >私信</a>
                                                </div>
                                            </div>
                                        </div>
...
<script type="text/javascript">
    ...
    function searchTabs(value){
    	var tag_href="${p}/train/to_train_detail.action?id="+${trainView.id}+"&jumpId=${trainView.jumpId}&jumpType=${trainView.jumpType}&sign="+value+"&classId="+${trainView.classId};
    ...
    function doSearchCourse(courseId,trainType){
		...
		if("${isTeacher}"==="true"&&"${trainDetailView.viewType }"==='1'){
			alert("预览模式不能打开培训课程");
			return false;
		}
        ...
        $("#train_course_"+trainType+"_"+courseId).attr("href","${p}/user_study/to_course_study.action?page=1&learnId="+courseId+"&sourceType=0&tab=1&is_direct=1&trainType="+trainType+"&id=${userStudyView.id}");
    }
