...
<div class="main_content">
			...
			<form id="stupid" action="${pageContext.request.contextPath}/back_user/to_user_active.action">
				...
				<div class="time">
					<div class="choose_date_01">
						...
						<a onclick="queryActiveByTimeType(3);" class="<c:if test="${timeType=='3'}">active</c:if>" href="javascript:void(0);" title="截止到当前日期">最近30天</a>
				...
				<div class="time">
					<div class="choose_date_01">
						...
						<a onclick="queryActiveBylogType('0');" title="每日在线时间超过${hour}小时的用户" class="<c:if test="${fn:contains(logType,'0')==true}">active</c:if>" href="javascript:void(0);">高活跃用户</a>
						<sub>(规定<input id="hour" value="${hour}" name="hour" type="text" style="width: 20px;" onfocus="selectHour();" />小时)</sub>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a onclick="queryActiveBylogType('1');" title="日在线时间不超过${hour}小时且未消失的用户" class="<c:if test="${fn:contains(logType,'1')==true}">active</c:if>" href="javascript:void(0);">尝试用户</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<div class="main_body">
		...
		<div class="time user_info">
			<div>
				<span title="每日在线时间未超过${hour}小时的用户">高活跃用户</span><br/>
				<label class="number" id="activeNum"></label>
			</div>
			<div>
				<span title="日在线时间不超过${hour}小时且未消失的用户">尝试用户</span><br/>
				<label class="number" id="tryNum"></label>
			</div>
			<div>
				<span title="间隔${oneDay}天再次登录的用户">回归僵尸用户</span><br/>
		...
		<div class="time" style="clear: both;">
			<table class="table_sta">
				...
				<tr>
					<th>日期</th>
					<th title="每日在线时间超过${hour}小时的用户">高活跃用户</th>
					<th title="日在线时间不超过${hour}小时且未消失的用户">尝试用户</th>
					<th title="间隔${oneDay}天再次登录的用户"> 回归僵尸用户</th>
				</tr>
