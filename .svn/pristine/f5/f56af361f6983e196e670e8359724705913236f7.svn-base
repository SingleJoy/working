...
public class BackFileAnalyzeServiceImpl implements BackFileAnalyzeService {
	...
	@Override
	public List<FileStatsView> queryFileRanking(FileStatsView fileStats) {
		List<FileStatsView> list = null;
		String group = fileStats.getGroup();
		if("0".equals(group)){//按资源分组
			list = fileBaseMapper.queryFileRanking0(fileStats);
		}else if("1".equals(group)){//按人分组
			list = fileBaseMapper.queryFileRanking1(fileStats);
		}
		return list;
	}
