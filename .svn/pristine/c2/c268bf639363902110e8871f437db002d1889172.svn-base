...
<body class="easyui-layout" data-options="fit:true">
	...
	<div style="font:13.3333px Arial;z-index:9999; position: absolute;margin-left:33px;margin-top: -40px;">
    ...
    <form  action="${pageContext.request.contextPath}/back_user_default_set/update_default_set.action" method="post" id="signature_form">
		...
		<div id="default_signature"  style="margin-left:-490px;margin-top: 60px;<c:if test="${type!=0}">display:none</c:if>">
		    <textarea id="signature" rows="8" cols="60" name="content"  onKeyUp="LimitTextArea(this,30);"  >${defaultUserSet.content}</textarea>
	...
	<form  action="${pageContext.request.contextPath}/back_user_default_set/update_default_set.action" method="post" id="profile_form">
		...
		<div id="default_profile"  style="margin-left:-524px;margin-top:140px;<c:if test="${type!=1}">display:none</c:if>">
			<textarea id="synopsis" rows="8" cols="60" name="content"  onKeyUp="LimitTextArea(this,200);" onblur="minnum();" >${defaultUserSet.content}</textarea>
		    <div class="help-block">字数请保持在15-200</div>
	        <div class="help-block" style="color:#F00" id="synopsis_msg" ></div>
		     <br><br><br>
			<div id="submit"  style="margin-left: -180px;">
...
<script type="text/javascript" charset="utf-8">
   ...
   function LimitTextArea(field,max){
       maxlimit=max;
